	...
	<xsl:template name="RNloop">
		...
		<xsl:choose>
			<xsl:when test="$RL_sigNset[$RL_iSigN] = $RL_rNset[$RL_iRN] and $RL_iRN != 0">
				<xsl:choose>
					<xsl:when test="$RL_formula = 'cterm' and $RL_rNset[$RL_iRN]/..//child::cterm">
						<_r n="{$RL_rNset[$RL_iRN]}">
							<xsl:apply-templates select="$RL_rNset[$RL_iRN]/../cterm"/>
						</_r>
					</xsl:when>
					<xsl:when test="$RL_rSet[$RL_iRN]/cterm">
						<_r n="{$RL_rNset[$RL_iRN]}">
							<xsl:apply-templates select="$RL_rSet[$RL_iRN]/cterm"/>
						</_r>
					</xsl:when>
					<xsl:otherwise>
