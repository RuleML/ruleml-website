 ...
 <div align="center"><img src="img/logo.jpg" alt="logo" border="0" height="42" width="320" align="BOTTOM"/> 
  ...
  -->
					...
					<xsl:otherwise>
						...
						<ul>
							<li><xsl:apply-templates select="uplink"/></li>
						</ul>
	...
	<a href="{@href}"><i><b>Up to Parent Page</b></i></a>
	</xsl:template>
  ...
  # 
 <xsl:choose><xsl:when test="@label"><xsl:value-of select="@label"/></xsl:when><xsl:otherwise><xsl:value-of select="header"/></xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:apply-templates select="header" mode="header"/>
			</a>
		</li>
	</xsl:template>
