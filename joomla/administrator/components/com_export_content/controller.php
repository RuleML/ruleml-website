<?php
/**
 * @version		$Id: controller.php 9097 2007-10-01 15:54:19Z nurainir $
 * @package		Joomla
 * @subpackage	Content
	...
	*/
	function viewContent()
	{
		...
		$query = 'SELECT cc.id AS value, cc.title AS text, section' .
				...
				' INNER JOIN #__export_sections AS s ON s.id = cc.section ' .
				$filter .
	...
	function editContent($edit)
	{
		...
		if ($id)
		{
			...
			$query = 'SELECT COUNT(content_id)' .
					' FROM #__export_content_frontpage' .
	...
	*/
	function moveSection()
	{
		...
		$query = 'SELECT CONCAT_WS( ", ", s.id, c.id ) AS `value`, CONCAT_WS( " / ", s.title, c.title ) AS `text`' .
				' FROM #__export_sections AS s' .
				' INNER JOIN #__export_categories AS c ON c.section = s.id' .
