...
englishStartTime,englishEndTime,englishStartDate,englishEndDate,englishLocation,englishFitness;
...
function elementSelectedFitness(){
	...
	if(choice > 0){
		...
		englishFitness = "any fitness level";
	}
...
function elementErrorDurationCheck(){
	choice = parseInt(document.form.durationHours.value);
	choice2 = parseInt(document.form.durationMinutes.value);
	if(!(choice >= 0) || !(choice2 >= 0)){
		if (!(choice >= 0)) document.form.durationHours.className='redField';
		if (!(choice2 >= 0)) document.form.durationMinutes.className='redField';
	}
	else{
